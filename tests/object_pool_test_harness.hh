...
public:
    ...
    void return_object(const T& obj) {
        this->execute(Return<T>(obj));
    }

    void expect_size(size_t expected) {
        this->execute(PoolSize<T>(expected));
    }

    void expect_available(size_t expected) {
        this->execute(AvailableCount<T>(expected));
    }

    void expect_borrowed(size_t expected) {
        this->execute(BorrowedCount<T>(expected));
    }
}; 