...
public:
  ...
  // 构造函数
  explicit GeneralObjectPool(
    Creator creator,
    Resetter resetter = []( Object& ) {},
    size_t initial_size = 0,
    size_t max_size = std::numeric_limits<size_t>::max() )
    : max_size_( max_size ), pool_(), creator_( std::move( creator ) ), resetter_( std::move( resetter ) ), mutex_()
  {
    // 预分配对象
  ...
  Ref<Object> borrow()
  {
    ...
    // 尝试找到未借出的对象
    auto it = std::find_if(
      pool_.begin(), pool_.end(), []( const ObjectWrapper& wrapper ) { return !wrapper.is_borrowed; } );
  ...
  void return_object( const Object& obj )
  {
    std::lock_guard<std::mutex> lock( mutex_ );
    auto it = std::find_if(
      pool_.begin(), pool_.end(), [&obj]( const ObjectWrapper& wrapper ) { return &wrapper.obj == &obj; } );
